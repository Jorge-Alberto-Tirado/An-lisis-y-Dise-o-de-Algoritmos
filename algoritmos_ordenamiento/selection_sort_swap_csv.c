...
int main(){
    ...
    // CICLO 2: Variar el tamaño del problema (k) Ejemplo: 100, 200, 300...
    for(int k = paso; k <= max_n; k += paso){
        ...
        for(int rep = 0; rep < 30; rep++){
            ...
            // CICLO 4: Copiar los datos base al arreglo de trabajo. Esto evita modificar el arreglo original
            for(int i = 0; i < k; i++){
                v[i] = datos[i];
            }
            ...
            // CICLO 5: Algoritmo principal (selection sort por maximos) En cada iteracion se coloca el mayor al final del segmento no ordenado
            for(int i = 0; i < k - 1; i++){

                int idx_max = 0;

                // CICLO 6: Buscar el elemento maximo en el rango [0, k-i) Cada vuelta reduce el rango porque el final ya esta ordenado
                for(int j = 1; j < k - i; j++){
                    if(v[j] > v[idx_max]){
                        idx_max = j;
                    }
                ...
                // INTERCAMBIO (SWAP) Se coloca el maximo en la posicion final del segmento
                int aux = v[k - 1 - i];
                v[k - 1 - i] = v[idx_max];
                v[idx_max] = aux;
            }
