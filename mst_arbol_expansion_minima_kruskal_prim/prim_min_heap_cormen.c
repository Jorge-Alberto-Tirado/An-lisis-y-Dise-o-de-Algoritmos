...
struct MinHeap {
    int size;
    int capacity;
    int *pos; // pos[v] guardara el indice actual del vertice v en el arreglo
    struct MinHeapNode *array;
};
...
void swapMinHeapNode(struct MinHeap *minHeap, int i, int j) {
    ...
    // 1. Intercambiamos sus registros en el arreglo de posiciones
    minHeap->pos[minHeap->array[i].v] = j;
    minHeap->pos[minHeap->array[j].v] = i;

    // 2. Intercambiamos los nodos fisicos en el arreglo del heap
...
struct MinHeapNode extractMin(struct MinHeap *minHeap) {
    ...
    // Movemos el ultimo nodo a la raiz
    minHeap->array[0] = lastNode;

    // Actualizamos el mapa de posiciones
    minHeap->pos[root.v] = -1; // El extraido ya no esta en el heap
...
void decreaseKey(struct MinHeap *minHeap, int v, int new_key) {
    ...
    // Buscamos en que indice esta nuestro vertice en O(1) usando pos
    int i = minHeap->pos[v];

    minHeap->array[i].key = new_key;

    // Burbujeamos hacia arriba mientras sea menor que su padre
    while (i > 0 && minHeap->array[i].key < minHeap->array[(i - 1) / 2].key) {
        instrucciones++;
...
// Funcion para verificar si un vertice sigue en el heap
int isInMinHeap(struct MinHeap *minHeap, int v) {
    if (minHeap->pos[v] < minHeap->size && minHeap->pos[v] >= 0)
        return 1;
    return 0;
}
...
int primCormenHeap(int matriz[MAX][MAX], int vertices) {
    ...
    // La capacidad ahora es exactamente V, ni un nodo mas, adios duplicados
    struct MinHeap *minHeap = createMinHeap(vertices);

    // Inicializamos todos los vertices al infinito, excepto el origen
    for (int v = 1; v < vertices; ++v) {
        instrucciones++;
        padre[v] = -1;
        key[v] = INT_MAX;
        minHeap->array[v].v = v;
        minHeap->array[v].key = INT_MAX;
        minHeap->pos[v] = v;
    }

    // Configuramos el vertice 0 como punto de partida
    key[0] = 0;
    minHeap->array[0].v = 0;
    ...
    while (minHeap->size > 0) {
        ...
        // Extraemos el vertice con la conexion mas barata
        struct MinHeapNode minNode = extractMin(minHeap);
        int u = minNode.v;

        // Revisamos todos los vecinos del vertice u
        for (int v = 0; v < vertices; ++v) {
            instrucciones++;
            
            // Si hay conexion, el vecino sigue en el heap, y el costo es mejor
            if (matriz[u][v] > 0 && isInMinHeap(minHeap, v) && matriz[u][v] < key[v]) {
                key[v] = matriz[u][v];
                padre[v] = u;
                
                // Actualizamos su prioridad en el heap en lugar de duplicarlo
                decreaseKey(minHeap, v, key[v]);
            }
        }
    }

    // Calculamos el costo total del MST
    int costoTotal = 0;
    for (int i = 1; i < vertices; ++i) {
        if (padre[i] != -1 && matriz[i][padre[i]] > 0) {
            costoTotal += matriz[i][padre[i]];
...
int main()
{
         ...
         (condicion_subida ? (vertices += intervalo) : (vertices -= intervalo)))
    {
        ...
        for (int r = 0; r < REPETICIONES; r++)
        {
            ...
            costo = primCormenHeap(matriz_global, vertices);
